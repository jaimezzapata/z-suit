...
export default function CoursesPage() {
  ...
  const togglePublic = async (courseId, currentStatus) => {
    ...
    if (result.success) {
      toast.success(
        currentStatus ? "Curso ahora es privado" : "Curso ahora es público"
      );
      await loadCourses();
    }
  };
    ...
    <div className="min-h-screen bg-[var(--bg-darkest)]">
      ...
      <main className="max-w-7xl mx-auto px-8 py-8">
        {loading ? (
          ...
          <>
            {/* Agrupar cursos por año y nivel */}
            {Object.entries(
              courses.reduce((groups, course) => {
                ...
                const nivel = course.nivel || "1";
                if (!groups[year][nivel]) groups[year][nivel] = [];
                groups[year][nivel].push(course);
                return groups;
              }, {})
            )
              .sort(([yearA], [yearB]) => yearB - yearA) // Ordenar años descendente
              .map(([year, nivelesGroup]) => (
                  ...
                  <div className="mb-4 pb-2 border-b border-[var(--border-color)]">
                    <h2 className="text-2xl font-bold text-[var(--text-primary)] flex items-center gap-2">
                      ...
                      <span className="text-sm text-[var(--text-secondary)] font-normal">
                        ({Object.values(nivelesGroup).flat().length} cursos)
                      </span>
                    </h2>
                  </div>

                  {/* Niveles dentro del año */}
                  {Object.entries(nivelesGroup)
                    .sort(([nivelA], [nivelB]) => nivelA - nivelB) // Ordenar niveles ascendente
                    .map(([nivel, coursesInNivel]) => (
                      <div key={`${year}-${nivel}`} className="mb-6">
                        {/* Header del Nivel */}
                        <h3 className="text-lg font-semibold text-[var(--text-secondary)] mb-3 flex items-center gap-2">
                          <span className="px-3 py-1 bg-[var(--accent-primary)]/20 text-[var(--accent-primary)] text-sm rounded-full">
                            Nivel {nivel}
                          </span>
                          <span className="text-sm font-normal">
                            ({coursesInNivel.length}{" "}
                            {coursesInNivel.length === 1 ? "curso" : "cursos"})
                          </span>
                        </h3>

                        {/* Grid de cursos */}
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                          {coursesInNivel.map((course) => {
                            const sessionsCompleted =
                              course.sessions?.filter((s) => s.documentation)
                                .length || 0;
                              ...
                              >
                                <div className="flex items-stretch">
                                  ...
                                  <div className="w-48 bg-[var(--bg-medium)] border-l border-[var(--border-color)] p-4 flex flex-col gap-2">
                                    ...
                                    <button
                                      onClick={() =>
                                        togglePublic(course.id, course.isPublic)
                                      }
          ...
          <div className="flex-1 grid grid-cols-2 gap-6 overflow-hidden">
            ...
            <div className="bg-[var(--bg-darkest)] rounded-lg p-4 overflow-y-auto border border-[var(--border-color)]">
                  ...
                  <div className="flex-1 p-3">
                    ...
                    <div className="flex items-center gap-4 text-xs">
                      <div className="flex items-center gap-1 text-[var(--text-secondary)]">
                        ...
                        <span>
                          0/{formData.tipo === "empresarial" ? "7" : "18"}{" "}
                          sesiones
                        </span>
                      </div>
                      <div className="flex items-center gap-1 text-[var(--text-secondary)]">
              ...
              <div className="mt-3 p-2 bg-[var(--bg-medium)] rounded text-xs text-[var(--text-secondary)]">
                💡 Se generará automáticamente un código de acceso único al
                crear el curso
              </div>
            </div>
          </div>
